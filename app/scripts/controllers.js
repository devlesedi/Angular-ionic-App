...
.controller('ItemCtrl', function($scope, Utils, IonicComponent, ItemConfig, Promise) {
	...
	Promise.resolve({method: 'GET', url: '/api/cobject/v0/item/'+Utils.Params.objectId})
      .then(function(response){
  ...
  $scope.getPhoto = function() {
	...
	$scope.publishItem = function(){
		...
		if(validate){
			...
			formDataResult.append('hiringCompany', $scope.publish.companyName)
			formDataResult.append('hiringCompanyWebsite', $scope.publish.companySite)
			formDataResult.append('salaryMin', $scope.publish.minSalaray)
			formDataResult.append('salaryMax', $scope.publish.maxSalary)
			formDataResult.append('salaryType', $scope.publish.salaryRate)
			...
			formDataResult.append('published', false)
